class Api::V1::SearchTermsController < ApplicationController
  def fetch_data
    if params[:query]
      @businesses = Business.where("name like '#{ params[:query] }%' and industry_id IS NOT NULL").limit(6)
      selected_industries = @businesses.pluck(:industry_id)
      @industries = Industry.where("name like '#{ params[:query] }%' and id NOT IN (?)", selected_industries).limit(8 - @businesses.count)
